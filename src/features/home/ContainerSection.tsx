...
const STAGES: DeployStage[] = [
  {
    id: 'build',
    title: 'Изграждане на image',
    description:
      'Файлът Dockerfile нарежда приложението върху Node.js изпълнителна среда и базова операционна система. Всека инструкция става кэшираем слой.',
    icon: Package,
    substeps: [
      'FROM node:20-alpine — базова ОС',
      'COPY package*.json и npm ci',
      'Копиране на изходния код в image',
      'Резултат: преносим Docker image',
    ],
  },
  {
    id: 'registry',
    ...
    description:
      'Имиджът се етикетира, подписва и изпраща към централизиран регистър — библиотека, от която всеки сървър може да изтегли имиджа.',
    icon: Cloud,
    substeps: [
      'Етикетиране с версия / git SHA',
      'Пускане към GitHub Container Registry',
      'Подписано с cosign за сигурност',
    ],
  },
  {
    id: 'deploy',
    title: 'Разгръщане и балансиране',
    description:
      'Оркестраторът изтегля имиджа и стартира контейнери. Load balancer разпределя трафика между здравите копия.',
    icon: Server,
    substeps: [
      'Изтегляне на имиджа от регистъра',
      'Стартиране на 3 копия на контейнер',
      'Load balancer насочва чрез round-robin',
    ],
  },
  {
    id: 'scale',
    title: 'Самовъзстановяване и мащабиране',
    description:
      'Здравни проверки на всеки контейнер. Провалилите се убиват и автоматично заменят — нулев престой.',
    icon: HeartPulse,
    substeps: [
      'HTTP здравна проверка всеки 10с',
      '3 неуспеха → подът се унищожава',
      'Замяната стартира автоматично',
    ],
  },
];
...
export function ContainerSection() {
  ...
  return (
    <section ref={setRef} className="section-spacing mx-auto max-w-6xl px-6">
      ...
      >
        <h2 className="mb-3">Мащабиране с контейнери</h2>
        <p className="mx-auto max-w-xl text-muted-foreground">
          От един лаптоп до хиляди потребители — как RedRep остава бърз
          с Docker и автоматизирано управление на контейнери.
        </p>
      </motion.div>
      ...
      {activeStage < 0 && (
        ...
        >
          ...
          <p className="text-sm text-muted-foreground">
            Кликни етап от долу, за да видиш процеса на разгръщане в действие.
          </p>
        </motion.div>
      )}
      ...
      >
        ...
        <p className="text-xs text-muted-foreground leading-relaxed">
          ...
          Един commit се изгражда, изпраща и разгръща на множество контейнери.
          Ако някой се провали, оркестраторът го заменя — потребителите ви не забелязват нищо.
        </p>
      </motion.div>
    </section>
  );
